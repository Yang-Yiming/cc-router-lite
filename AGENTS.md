...
A lightweight Rust CLI tool (`ccrl`) for switching between multiple Codex API profiles. Injects env variables into `~/.Codex/settings.json` or exports them to the current shell.
...
- Settings: `~/.Codex/settings.json` (hardcoded, no `settings_file` config)
